...
export function PreviewScanStrip({ presetId, renderingUrl, qrUrl, componentsText, datamodelText }: PreviewScanStripProps) {
  ...
  useEffect(() => {
    ...
    try {
      setPreview({
        components: JSON.parse(componentsText) as A2uiPayload,
        datamodel: datamodelText.trim() ? JSON.parse(datamodelText) as A2uiPayload : null,
      });
      setParseError(null);
    } catch (cause) {
      setParseError(cause instanceof Error ? cause.message : "Invalid JSON");
    }
  }, [componentsText, datamodelText]);
  ...
  if (!qrUrl && !presetId && !preview && !parseError) return null;
    ...
    <div className="shrink-0 border-b border-slate-200 bg-white p-2.5">
      <div className="flex items-stretch gap-2.5">
        <div className="relative h-[224px] min-w-0 flex-1 overflow-hidden rounded-lg border border-slate-200 bg-slate-50/70">
          {preview ? (
            <A2uiPreview components={preview.components} datamodel={preview.datamodel} referenceSize={referenceSize} fit="width" onAction={(payload) => setAction(JSON.stringify(payload))} />
          ) : renderingUrl ? (
              <button
                type="button"
      ...
      {parseError && <p role="alert" className="mt-1 text-[10px] text-amber-700">Invalid JSON; showing the last valid preview. {parseError}</p>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-8 backdrop-blur-sm" onClick={() => setExpandedPreview(false)}>
          <div role="dialog" aria-modal="true" aria-label="Resizable preview" className="relative h-[680px] w-[760px] min-h-[280px] min-w-[320px] max-h-[calc(100vh-4rem)] max-w-[calc(100vw-4rem)] resize overflow-hidden rounded-xl border border-slate-200 bg-slate-50 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="h-[calc(100%-2.5rem)]">
              <A2uiPreview components={preview.components} datamodel={preview.datamodel} referenceSize={referenceSize} onAction={(payload) => setAction(JSON.stringify(payload))} />
            </div>
          </div>
        </div>
      )}
