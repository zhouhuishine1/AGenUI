      ...
      return (0, _assign.default)({}, acc, {
        ...
        platforms: {
          ...
          ...(selectedPlatform && config.platforms[selectedPlatform] ? {
            [selectedPlatform]: config.platforms[selectedPlatform]
          } : !selectedPlatform ? config.platforms : undefined)
        },
...
async function loadConfigAsync({
  ...
  let lazyProject;
  const userConfig = await (0, _readConfigFromDisk.readConfigFromDiskAsync)(projectRoot);
  const initialConfig = {
    root: projectRoot,
    get reactNativePath() {
      return userConfig.reactNativePath ? _path().default.resolve(projectRoot, userConfig.reactNativePath) : (0, _resolveReactNativePath.default)(projectRoot);
  ...
  const finalConfig = await Array.from(new Set([...Object.keys(userConfig.dependencies), ...(0, _findDependencies.default)(projectRoot)])).reduce(async (accPromise, dependencyName) => {
    const acc = await accPromise;
    const localDependencyRoot = userConfig.dependencies[dependencyName] && userConfig.dependencies[dependencyName].root;
    try {
      let root = localDependencyRoot || (0, _cliTools().resolveNodeModuleDir)(projectRoot, dependencyName);
      let config = await (0, _readConfigFromDisk.readDependencyConfigFromDiskAsync)(root, dependencyName);
      return (0, _assign.default)({}, acc, {
        dependencies: (0, _assign.default)({}, acc.dependencies, {
          get [dependencyName]() {
            return getDependencyConfig(root, dependencyName, finalConfig, config, userConfig);
...
//# sourceMappingURL=/Users/szymonrybczak/Developer/cli/packages/cli-config/build/loadConfig.js.map